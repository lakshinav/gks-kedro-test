...
from operator import methodcaller # to calculate all the measures at once

def predict_model(
    model, test_y:pd.Series, n:int, parameters: Dict[str, Any]
    ):
    
    return model.predict(n-len(test_y)+1, n)


def eval_measures(
    predict_y:pd.Series, test_y:pd.Series, 
    ):
    eval_measures = (rmse, rmspe, maxabs, meanabs, medianabs, bias, medianbias, vare, stde)
    
    em_test = list(map(methodcaller('__call__', test_y, predict_y), eval_measures))
    #return em_test
    log = logging.getLogger(__name__)
    #log.info('Model {0}: {1:5.2f}'.format(eval_measures[1].__name__, em_test[1]))
    for i in range(len(em_test)):
        log.info('Model {0}: {1:5.2f}'.format(eval_measures[i].__name__, em_test[i]))
